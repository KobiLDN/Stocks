...
2. **Preview** — Cloudflare Pages auto-deploys `dev` in ~30–60s → preview URL appears in Cloudflare Pages → Deployments (viewable from any device). Local `file://` still works instantly for quick checks without pushing.
