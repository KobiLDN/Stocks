#!/usr/bin/env python3
"""
Generate the Defence Top 10 Picks signals via OpenRouter (deepseek/deepseek-v4-flash).
...
SYSTEM_PROMPT = """You are an investment research assistant analysing a curated universe of defence and aerospace stocks for a UK-based research dashboard. Output is for informational and educational purposes only — NOT financial advice.

The universe covers six categories: US primes (major US defence contractors), UK/European (BAE Systems, Rolls-Royce, QinetiQ), cyber-intel (Palantir, Leidos, CACI, SAIC, Booz Allen), drones/autonomy (AeroVironment, Kratos, Red Cat, Textron), space defence (Rocket Lab, Planet Labs, AST SpaceMobile), and weapons/systems (TransDigm, HEICO, Axon, Olin, Mercury Systems, Leonardo DRS).
...
Before ranking, use web search to find the latest news, analyst upgrades, earnings surprises, contract wins, or deal announcements for the highest-momentum stocks listed in the user prompt. Use what you find to enrich your rationale with live catalysts.
...
Pick the 10 most attractive stocks RIGHT NOW based on a BALANCED view of:
  ...
  - Defence-specific factors: contract wins and programme awards, government budget cycles and spending announcements, geopolitical risk and escalation events, export licence approvals, earnings beats on defence revenues, backlog growth
...
def call_api(api_key, system_prompt, user_prompt):
    headers = {
        ...
        "HTTP-Referer":  "https://kobildn.github.io/Stocks/Defence/",
        "X-Title":       "Defence Dashboard - Signals",
    }
    body = {
        "model":           MODEL,
        "messages": [
