#!/usr/bin/env python3
"""
Generate the Energy Top 10 Picks signals via OpenRouter (deepseek/deepseek-v4-flash).
...
Run locally:  python Energy_generate_signals_local.py
CI:           triggered by generate-signals.yml (weekly + workflow_dispatch)

Inputs:  prices.json   (written by Energy_update_prices.py / GitHub Actions)
...
def load_input():
    try:
        ...
        fail(f"{INPUT_JSON} not found. Run Energy_update_prices.py first.")
...
SYSTEM_PROMPT = """You are an investment research assistant analysing a curated universe of energy sector stocks for a UK-based research dashboard. Output is for informational and educational purposes only — NOT financial advice.
...
Pick the 10 most attractive stocks RIGHT NOW based on a BALANCED view of:
  ...
  - Fundamentals (P/E, market cap, beta, dividend yield — energy stocks often pay meaningful dividends)
  ...
  - Energy-specific factors: crude oil and natural gas price moves, OPEC+ production decisions, refining margins and crack spreads, LNG demand and export capacity, renewable energy capacity additions and power purchase agreements, interest rate sensitivity for utilities and clean energy, carbon pricing and ESG policy trends, geopolitical risk in energy supply chains
  ...
  "picks": [
    {
      ...
      "confidence": 0.0,
      "rationale": "1-2 sentences citing specific numbers from the input (e.g. '+12% YTD, P/E 14, dividend yield 4.2%')",
...
def call_api(api_key, system_prompt, user_prompt):
    headers = {
        ...
        "HTTP-Referer":  "https://kobildn.github.io/Stocks/Energy/",
        "X-Title":       "Energy Dashboard - Signals",
    }
    body = {
        "model":           MODEL,
        "messages": [
