#!/usr/bin/env python3
"""
Generate the Technology Top 10 Picks signals via OpenRouter (deepseek/deepseek-v4-flash).
...
SYSTEM_PROMPT = """You are an investment research assistant analysing a curated universe of technology stocks for a UK-based research dashboard. Output is for informational and educational purposes only — NOT financial advice.

The universe covers six categories: mega-cap (Apple, Microsoft, Alphabet, Amazon, Meta), semiconductors (NVIDIA, AMD, Intel, TSMC, Broadcom, Qualcomm, ARM, ASML, Micron, Applied Materials), enterprise software (Oracle, Salesforce, Adobe, ServiceNow, Snowflake, Intuit), cybersecurity (CrowdStrike, Palo Alto Networks, Cloudflare, Fortinet), consumer/platform (Netflix, Shopify, Spotify, Uber), and hardware/infrastructure (Cisco, Palantir).
...
Before ranking, use web search to find the latest news, analyst upgrades, earnings surprises, product launches, or deal announcements for the highest-momentum stocks listed in the user prompt. Use what you find to enrich your rationale with live catalysts.
...
Pick the 10 most attractive stocks RIGHT NOW based on a BALANCED view of:
  ...
  - Tech-specific factors: AI product launches and adoption, cloud revenue growth, earnings beats, market share gains, regulatory developments, M&A activity
...
def call_api(api_key, system_prompt, user_prompt):
    headers = {
        ...
        "HTTP-Referer":  "https://kobildn.github.io/Stocks/Tech/",
        "X-Title":       "Technology Dashboard - Signals",
    }
    body = {
        "model":           MODEL,
        "messages": [
