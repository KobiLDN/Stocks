...
│   ├── prices-data.js  ← live price data (auto-generated by update_prices.py)
│   ├── update_prices.py← fetches prices from Yahoo Finance → writes prices-data.js
│   └── ...
...
`prices-data.js` is the single source of truth for all live data. It is generated by `update_prices.py` and loaded by every page in its sector.
...
1. **`update_prices.py`** — add one entry to `STOCKS` dict: `"TICKER": ("yahoo_sym", "category", "EXCHANGE", special_or_None, "Company Name")`
...
4. Run `update_prices.py` — `prices-data.js` regenerates and all pages update automatically
