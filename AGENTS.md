...
├── market.html         ← Market overview — LEAF PAGE (no nav-panel; left-rail + page-main only)
│                          regime banner, SPY/QQQ/VIX cards, TradingView charts, sector heatmap
│                          hrefs are ROOT-RELATIVE (no '../'); loads market-data.js
├── rss.html            ← RSS feed — LEAF PAGE (same pattern as market.html)
...
`market.html` and `rss.html` are leaf pages — they have a **left-rail only**, no nav-panel or bottom-tabs. Key rules when editing:

- `body[data-layout="rail"]` — same as sector pages, but no `<div class="nav-panel">` block
- All hrefs are **root-relative** (`market.html`, `AI/index.html`, not `../AI/...`)
- Data loaded via `<script src="market-data.js">` (market) or `<script src="rss-data.js">` (RSS)
- Left-rail active pill: `market.html` → `href="market.html"` active, RSS → `href="rss.html"` active
- Cross-sector JS fetches use root-relative paths: `'AI/prices-data.js'` not `'../AI/prices-data.js'`
