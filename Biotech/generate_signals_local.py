#!/usr/bin/env python3
"""
Generate the Biotech Top 10 Picks signals via OpenRouter (deepseek/deepseek-v4-flash).
...
SYSTEM_PROMPT = """You are an investment research assistant analysing a curated universe of biotech and pharmaceutical stocks for a UK-based research dashboard. Output is for informational and educational purposes only — NOT financial advice.
...
Before ranking, use web search to find the latest news, FDA decisions, clinical trial results, analyst upgrades, or deal announcements for the highest-momentum stocks listed in the user prompt. Use what you find to enrich your rationale with live catalysts.
...
Pick the 10 most attractive stocks RIGHT NOW based on a BALANCED view of:
  ...
  - Biotech-specific catalysts: pipeline progress, FDA approvals/PDUFA dates, clinical trial results, platform technology strength
  - Sector risk factors: binary clinical event risk, regulatory approval timelines, patent cliffs, competitive pipeline threats

Categories in this universe: large-cap (established big pharma/biotech), uk-listed (LSE-traded), gene-editing (CRISPR/base editing), genomics (sequencing/diagnostics), oncology (cancer therapeutics), mrna (mRNA/vaccine platforms), rare-disease (orphan drugs), metabolic (diabetes/obesity/cardio), neuroscience (CNS/neurological).
...
def call_api(api_key, system_prompt, user_prompt):
    headers = {
        ...
        "HTTP-Referer":  "https://kobildn.github.io/Stocks/Biotech/",
        "X-Title":       "Biotech Dashboard - Signals",
    }
    body = {
        "model":           MODEL,
        "messages": [
