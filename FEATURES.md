...
- **Hub Top 20 Signals panel + 1920px layout** — full-width panel above sector cards shows top 20 picks across all 6 sectors ranked by confidence; all 9 columns sortable (▼/▲, numeric data-val, STRONG BUY above BUY); sector grid changed from 2-col to 3-col (3+3); header/main/footer unified at 1920px max-width; responsive: 2-col ≤ 1100px, 1-col ≤ 640px; card gap 28px (`index.html`)

- **T212 MCP server + local portfolio dashboard** — `t212_mcp/server.py` (FastMCP); 4 tools: `get_positions`, `get_account_summary`, `get_orders`, `get_portfolio_vs_signals`; `.claude/settings.json` wires it into Claude Code automatically; `generate_export.py` merges T212 positions into `exports/YYYY-MM-DD-portfolio.json` (gitignored, local only). Container-level `G:\My Drive\coding\ai\Stocks\t212_mcp\portfolio.py` + `run.bat` — fetches live T212 positions, merges with latest STOCKSMain export, writes `snapshot.html` (1920px browser dashboard: squarified treemap heatmap sized by 1D% / coloured by direction; sector allocation donut; portfolio value line chart; Today's Movers; sortable holdings table with Bought+Current columns; sortable/filterable picks table) + `snapshot.json` + `snapshot.txt`; appends to `portfolio_history.json` each run to power the chart. `TICKER_ALIASES` maps T212 non-standard tickers to universe tickers. Key stored as `KEY_ID:SECRET_KEY` in `.key` outside any git repo
