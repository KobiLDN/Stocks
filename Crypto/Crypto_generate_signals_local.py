#!/usr/bin/env python3
"""
Generate the Crypto Top 10 Picks signals via OpenRouter (deepseek/deepseek-v4-flash).
...
Run locally:  python Crypto_generate_signals_local.py
CI:           triggered by generate-signals.yml (weekly + workflow_dispatch)

Inputs:  prices.json   (written by Crypto_update_prices.py / GitHub Actions)
...
def load_input():
    try:
        ...
        fail(f"{INPUT_JSON} not found. Run Crypto_update_prices.py first.")
...
The universe covers six categories: bitcoin (BTC — the reference asset), layer1 (major smart-contract L1s: ETH, SOL, BNB, ADA, AVAX), payments (payment/settlement chains: XRP, XLM, TRX), emerging (newer/niche L1s: SUI, HBAR, TON, ALGO, MINA), infra (scaling and infrastructure: POL, DOT, LINK), and meme (DOGE, PEPE).

You will be given a JSON snapshot of every tracked asset: prices in USD, year-to-date and 1-year returns, short-term momentum (1D/1W/1M %), fundamentals where available (market cap in $B, beta), recent news headlines and an aggregate news sentiment score in [-1, +1]. Note that P/E, dividend yield, short interest, and analyst ratings are typically only available for the equity category.
  ...
  "picks": [
    {
      ...
      "confidence": 0.0,
      "rationale": "1-2 sentences citing specific numbers from the input (e.g. '+45% YTD, beta 1.8, positive news sentiment 0.32')",
...
def call_api(api_key, system_prompt, user_prompt):
    headers = {
        ...
        "HTTP-Referer":  "https://kobildn.github.io/Stocks/Crypto/",
        "X-Title":       "Crypto Dashboard - Signals",
    }
    body = {
        "model":           MODEL,
        "messages": [
