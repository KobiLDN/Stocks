#!/usr/bin/env python3
"""
Generate the Global Mega-Cap Leaders Top 10 Picks signals via OpenRouter (deepseek/deepseek-v4-flash).
...
Run locally:  python MegaCap_generate_signals_local.py
CI:           triggered by generate-signals.yml (weekly + workflow_dispatch)

Inputs:  prices.json   (written by MegaCap_update_prices.py / GitHub Actions)
...
def load_input():
    try:
        ...
        fail(f"{INPUT_JSON} not found. Run MegaCap_update_prices.py first.")
...
SYSTEM_PROMPT = """You are an investment research assistant analysing a curated universe of global mega-cap stocks for an international research dashboard. Output is for informational and educational purposes only — NOT financial advice.

The universe covers seven categories: Big Tech (Apple, Microsoft, NVIDIA, Alphabet, Meta, Amazon, Tesla, Netflix — dominant platform and AI companies with significant cloud, advertising, and consumer technology exposure), Financials (JPMorgan Chase, Bank of America, Berkshire Hathaway, Visa, Mastercard, Goldman Sachs, BlackRock — largest US banks, global payment networks, and leading asset managers), Healthcare (Eli Lilly, UnitedHealth Group, Johnson & Johnson, AbbVie, Merck, Thermo Fisher Scientific, Abbott Laboratories — pharmaceutical innovators, biotech leaders, medical device makers, and managed care providers), Consumer (Procter & Gamble, Coca-Cola, PepsiCo, McDonald's, Nike, Walmart, Costco — global consumer staples brands and dominant retail chains), Energy & Industrial (ExxonMobil, Chevron, Caterpillar, Honeywell, GE Aerospace, RTX Corp, Boeing — integrated energy majors and diversified industrial conglomerates spanning aerospace, defence, and advanced manufacturing), Tech & Semis (Advanced Micro Devices, Qualcomm, Broadcom, Taiwan Semiconductor, ASML Holding, Arm Holdings, Intel — chipmakers, semiconductor IP licensors, and critical semiconductor equipment suppliers), Global Growth (Saudi Aramco, Alibaba Group, SAP SE, Sony Group, Toyota Motor, Novo Nordisk, HSBC Holdings — international mega-caps across energy, e-commerce, enterprise software, consumer electronics, automotive, GLP-1 pharmaceuticals, and global banking).
...
Before ranking, use web search to find the latest news, analyst upgrades, earnings surprises, and deal announcements for the highest-momentum stocks listed in the user prompt. Use what you find to enrich your rationale with live catalysts.

Pick the 10 most attractive stocks RIGHT NOW based on a BALANCED view of:
  - Momentum (recent 1D/1W/1M moves, YTD)
  - Fundamentals (P/E, market cap, beta, dividend yield)
  ...
  - Macro factors: Fed rate decisions and their differential effect on growth stocks vs value/dividend payers; mega-cap earnings cycles and guidance revisions; AI infrastructure investment themes (data centres, power, chip supply chains); GLP-1 drug market dynamics and pipeline competition; semiconductor cycle — inventory destocking vs restocking, leading-edge vs mature nodes; consumer spending trends and the US consumer health backdrop; geopolitical risk affecting global supply chains (Taiwan Strait, Middle East, US-China trade policy); currency dynamics for non-USD earners (SAR peg, EUR/USD, JPY/USD, DKK/USD)
  ...
  "picks": [
    {
      ...
      "confidence": 0.0,
      "rationale": "1-2 sentences citing specific numbers from the input (e.g. '+22% YTD, P/E 28, AI capex tailwind')",
...
def call_api(api_key, system_prompt, user_prompt):
    headers = {
        ...
        "HTTP-Referer":  "https://stocks-4qw.pages.dev/MegaCap/",
        "X-Title":       "Global Mega-Cap Leaders - Signals",
    }
    body = {
        "model":           MODEL,
        "messages": [
...
def main():
    ...
    print("\nTop 10 picks:")
    for p in picks:
        print(f"  {p['rank']:>2}. {p['ticker']:<8}  {p['signal']:<10}  "
